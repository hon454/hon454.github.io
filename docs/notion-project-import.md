...
- 기간·소속·역할·기술을 본문 상단의 공통 표로 표시한다. 원문에서 소속이 비어 있던 항목은 `—`로 표시한다. 이후 사용자 확인에 따라 Vapor World는 이메진 템페스트 게임스튜디오, Brutal League는 누라임 게임즈로 보완했다.
- 본문의 순서, 담당 업무, 수상·출시 이력, 외부 다운로드 링크를 보존한다. 사용자 요청에 따라 만료 안내 문구는 제거한다. Notion 다단 이미지는 단일 열로, 콜아웃은 인용문으로 변환한다.
- 사용자 확인에 따라 Vapor World의 “인풋, 이동, 전 시스템 개발”만 “인풋, 이동, 전투 시스템 개발”로 수정한다. 카드 설명은 원문에 있는 내용으로 요약한다.
- `published`는 프로젝트 참여 시작 월을 기준으로 한다. 원문에 일자가 없으므로 사용자 요청에 따라 해당 월 안에서 일자를 분산해 표시·정렬 기준으로 사용하며, 실제 시작 일자를 뜻하지 않는다. 기존 프로젝트 순서는 유지한다. 원문의 참여 기간은 월 단위 그대로 표에 남긴다. CINEVStudio와 Shotloom의 날짜는 변경하지 않는다.
...
- YouTube 영상 16개를 임베드로 변환한다. Brutal League 영상 링크는 사용자가 유효하지 않음을 확인하여 제거했다. 외부 서비스의 지속적인 접근 가능성이나 모든 영상의 재생을 보장하지는 않는다.
- 길고양이 이야기 2·Night of the Dead·onAirVR Client 2.0·학도병의 편지·CircleVR·Vapor World는 `published`(출시됨), 나머지 신규 9개는 `archived`(보관됨)로 설정한다.
...
- 15개 프로젝트의 사용자 요청으로 제거한 만료 안내 3개를 제외한 원문 텍스트 157개 항목을 대조하고 로컬 이미지 53개를 디코딩했다.
