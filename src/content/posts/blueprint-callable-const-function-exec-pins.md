...
`BlueprintCallable`을 붙인 C++ 함수를 Blueprint에 배치했는데 흰색 실행 핀이 나타나지 않는 경우가 있었다.
...
처음에는 `BlueprintCallable`이면 반드시 실행 핀이 생기고, `BlueprintPure`일 때만 실행 핀이 사라진다고 생각하기 쉽다. UE 4.27에서는 `const` 함수에 한 가지 자동 규칙이 더 있었다.

## UE 4.27 UHT가 Pure로 판단하던 조건

당시 UnrealHeaderTool은 다음 조건을 모두 만족하는 함수를 자동으로 `BlueprintPure`로 표시했다.
...
즉 엔진이 함수 본문을 분석해 실제 side effect가 있는지 판정한 것이 아니다. C++ 선언의 `const`와 Blueprint로 내보낼 output의 존재를 근거로 정한 규칙이었다.
...
`const`를 제거해서 해결할 수도 있지만, 함수가 객체 상태를 변경하지 않는다면 C++ 계약까지 약하게 만들 이유는 없다. `const`는 유지하고 Blueprint 노드의 실행 의미만 명시하는 편이 의도가 분명하다.

## 언제 Pure가 적합한가

Pure 노드는 데이터가 필요한 지점에서 Blueprint compiler가 평가한다. 실행선이 없고 한 번만 호출된다는 보장도 없으므로 다음 조건에 잘 맞는 함수에 사용한다.
...
반대로 다음과 같은 함수에는 실행 핀이 있는 편이 안전하다.
...
원문은 “side effect를 보유했느냐에 따라 엔진이 실행 핀을 정한다”고 설명했다. API를 설계하는 기준으로는 맞는 방향이지만, **UE 4.27 UHT가 함수 본문을 분석한 것은 아니다.** 실제 자동 Pure 처리는 `const`, output, `BlueprintCallable`, `BlueprintPure=false` 여부만 확인했다.
