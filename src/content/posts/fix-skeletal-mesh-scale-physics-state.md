...
Scale을 `KINDA_SMALL_NUMBER`, 즉 `0.0001` 부근 이하로 설정하면 다음과 같은 warning도 볼 수 있었다.
...
당시 문제는 scale을 복원한 뒤 simulation을 시작하기 전에 Skeletal Mesh Component의 physics state를 다시 만들어 해결했다.
...
UE 4.27의 `UActorComponent::RecreatePhysicsState()` 구현도 동작이 단순하고 명확했다.
...
즉 기존 PhysX state를 제거하고, Component가 등록된 상태라면 현재 Component 설정과 transform을 기준으로 다시 생성한다. 이 사례에서는 축소된 시점의 body state가 남아 있는 문제를 scale 복원 이후의 상태로 재구성했다.

## 호출 순서

핵심은 원래 scale로 돌린 **뒤**, physics simulation을 켜기 **전**에 호출하는 것이다.
...
중요한 점은 **최종 scale과 collision 설정이 반영된 state를 만든 뒤 simulation을 시작하는 것**이다.

## 주의할 점

`RecreatePhysicsState()`는 가벼운 transform update가 아니다. body를 파괴하고 다시 만들기 때문에 다음 사항을 고려한다.
...
이 해결책은 이미 어긋난 physics state를 복구하는 방법이다. 원래 문제였던 frame popping을 숨기기 위해 scale을 거의 0으로 만드는 대신, animation pose·visibility·spawn 연출을 조정할 수 있다면 그쪽이 더 근본적인 해결이다. 다만 당시 프로젝트처럼 해당 흐름을 즉시 바꾸기 어려운 상황에서는 simulation 직전의 `RecreatePhysicsState()`가 유효한 우회책이었다.
