...
전환의 큰 이유 중 하나는 당시 Unreal Engine 기반 개발 프로세스의 반복 속도였다. AI 에이전트가 발전해도 그 능력을 실제 개발 속도 향상으로 연결하기 어려웠다. 기술 스택 선정과 초기 구조는 팀이 담당했고, 나는 선택된 Rust·Bevy·React·TypeScript 환경에서 기능을 구현하고 통합·검증했다. 구현 과정에서는 에이전트가 코드를 탐색하고 여러 파일을 수정한 뒤 빌드와 테스트로 확인하는 능력을 실제 작업에 적용했다.
...
직접 맡은 구현 범위와 실제 제품 화면, 일반화 코드는 [Shotloom 프로젝트 글](/projects/shotloom/)에 정리했다.
