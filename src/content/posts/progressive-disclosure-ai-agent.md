...
`AGENTS.md`의 참조 규칙을 발췌·번역했다. 경로는 설명용으로 단순화했다.

```md title="CINEV Studio — AGENTS.md 일부"
## 외부 지침은 필요할 때 읽기
...
항상 지켜야 할 핵심 규칙은 원래 지침에 남겼다. 가끔 찾아볼 문서를 나누는 것과 놓치면 안 되는 규칙을 남기는 것은 별개의 일이었다.
...
CINEV Studio의 후속 프로젝트인 Shotloom은 브라우저에서 장면과 샷을 편집하는 도구다.

이 프로젝트는 시작부터 AI 에이전트와 코딩할 것을 전제로 구조를 잡았다. CINEV Studio에서 커진 지침을 나누는 데 집중했다면, Shotloom에서는 무엇을 어디에 기록하고 어떻게 다시 찾을지까지 정했다.

장기적으로 유지하는 프로젝트 문서도 역할을 나눴다. 작업이 끝나도 필요한 지식과 결정 근거를 남겨 다시 참조할 수 있게 했다.

- Spec에는 어떤 동작을 기대하는지 남긴다.
- ADR에는 무엇을 결정했고 왜 그렇게 결정했는지 남긴다.
- TechDebt에는 알고도 남겨둔 구조적 문제와 정리할 조건을 남긴다.

`AGENTS.md`는 공통 규칙과 진입점을 안내하고 `MAP.md`는 질문을 해당 문서에 연결한다. 문서 작성 지침에는 기록을 남길 위치와 코드 변경에 맞춰 문서를 갱신하는 기준을 정했다. 공통으로 읽어야 할 문서도 있으므로, 파일이 분리되어 있다는 이유만으로 모두 선택 사항은 아니다.

두 파일에서 문서를 찾아가는 부분을 발췌·번역하면 다음과 같다.

```md title="Shotloom — AGENTS.md → MAP.md"
# AGENTS.md
...
- 제품 스펙은 어디에 있나? → docs/specs/
- 설계 결정 기록은 어디에 있나? → docs/adr/
- 기술 부채 기록은 어디에 있나? → docs/tech-debt/
- 문서 작성 기준은 어디에 있나? → docs/guidelines/documentation-standard.md
```
