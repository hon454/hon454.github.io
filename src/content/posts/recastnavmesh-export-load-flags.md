...
모든 지도나 새 게임에서 발생한 것은 아니며 저장 데이터가 특정 지도를 참조할 때만 재현됐다. 오류 문자열을 보면 메모리에 이미 존재하는 `RecastNavMesh` 객체가 패키지 export로도 발견됐지만, 현재 로드가 요구하는 flags를 갖추지 못한 상태다.

## 적용 범위

대상은 Unreal Engine에서 Recast 기반 내비게이션을 사용하는 경우다. 오류 경로에는 지도 패키지의 `RecastNavMesh-Default`가 직접 나타난다. 같은 load flags 오류라도 지목된 객체가 다르면 그 객체의 생성·저장 경로부터 별도로 조사해야 한다.

## 원인

문제가 된 지도에는 `Nav Mesh Bounds Volume`이 없었다. 이 상태가 저장된 내비게이션 객체와 맞물렸다. Unreal은 유효한 내비게이션 영역을 기준으로 Recast 내비게이션 데이터를 생성한다. 지도의 bounds를 제거하거나 내비게이션 데이터를 다시 저장하는 과정에서 패키지 안의 export와 런타임 생성 상태가 어긋난 것으로 판단했다.
...
문제가 된 지도에 `Nav Mesh Bounds Volume`을 추가해 이동 가능한 영역을 포함했다. 지도를 저장한 뒤 내비게이션 데이터를 다시 생성했다. 이후 새 빌드와 새 프로세스에서 같은 저장 데이터를 불러오자 크래시가 사라졌다.
...
`Nav Mesh Bounds Volume`을 다시 제거하면 저장 과정에서 생성된 내비게이션 객체가 달라지기도 한다. 오류를 숨기려고 load flags 검사를 우회하거나 저장 데이터를 무조건 폐기하면 패키지 상태 불일치의 원인이 남는다.
