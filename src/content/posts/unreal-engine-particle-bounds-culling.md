...
emitter의 입자는 화면 안에 남아 있는데 component bounds가 view frustum 밖으로 판단되는 순간 전체 particle system이 컬링됐다. 반투명·긴 trail·넓게 퍼지는 effect에서 실제 시각 범위와 계산된 bounds가 다르면 쉽게 드러난다.

## 적용 범위

Cascade `UParticleSystem`을 사용하는 UE4/UE5 프로젝트에 해당한다. Niagara는 bounds 설정 위치와 계산 방식이 다르므로 system/emitter properties의 bounds를 별도로 확인한다.

## 원인

renderer는 모든 particle을 매 frame 개별 확인하지 않고 component의 bounding box로 먼저 표시 여부를 판단한다. bounds가 실제 effect보다 작거나 origin에서 벗어나 있으면 카메라 각도에 따라 effect 전체가 사라진다.
...
Particle System의 Fixed Relative Bounding Box를 켜고, effect가 실제로 도달하는 최대 범위를 포함하도록 box를 설정한다.
...
값을 임의로 매우 크게 잡기보다 가장 과격한 emitter 상태, 이동하는 parent와 LOD를 포함해 필요한 범위를 측정한다.
...
너무 작은 bounds는 시각 오류를 만들고, 지나치게 큰 bounds는 보이지 않는 effect까지 렌더링 후보로 남겨 성능을 해친다. 원점이나 scale이 잘못된 effect를 bounds 확대만으로 숨기지 않는다.
