...
- **AI 에이전트 활용 개발** — 커스텀 에이전트 하네스를 운용하고 요구사항·명세·ADR·테스트·PR을 하나의 흐름으로 연결해 작업의 판단 근거와 검증 결과를 추적
...
- **Web·Native 단일 제품 모델** — 브라우저 프리뷰, CLI와 서비스가 동일한 `.shotloom` 번들과 Rust 타임라인 코어를 사용하고, 동일한 프로젝트 데이터와 타임라인 평가 결과를 바탕으로 동작하도록 구성했습니다.
- **Grimoire 기반 AI 에이전트 활용 개발 체계** — 개인 오픈소스 Grimoire를 포함한 커스텀 에이전트 하네스를 저장소 초기부터 운용했습니다. 요구사항을 Linear 이슈·명세·ADR·테스트로 구조화하고 계획·구현·검증·PR을 하나의 흐름으로 연결했습니다. Shotloom 기간의 개인 변경 기준 주간 병합 빈도는 평균 16.1건으로, CINEVStudio 기간 평균 8.2건 대비 약 2배를 기록했습니다.
...
- **Action 도메인** — Action·Prop 데이터 규칙과 Sequencer·MovieScene 실행 흐름을 담당하고, UMG 타임라인에서 카메라·애니메이션·Root Motion·IK를 편집·재생하도록 연결했습니다. 기획·TA·QA와 기능·데이터 경계를 조율하고 반복 오류를 공통 규칙과 검증 도구에서 해결했습니다.
...
- **onAirVR 실시간 스트리밍** — 스튜디오 클라이언트 구현을 주도하고, PC에서 렌더링·인코딩한 스테레오 영상·오디오를 모바일 HMD로 전송하며 HMD 자세·입력을 서버로 되돌리는 소켓 경로와 세션별 카메라 리그를 Unity·C#로 개발했습니다.
...
- **아주대학교 미디어프로젝트 자문·멘토** — 2023 — 2026 · 5개 학기 참여, 2026년 2학기(2026.10 — 2026.11) 예정
